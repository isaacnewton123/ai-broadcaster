"""
poster_telegram.py — Poster generator khusus Telegram Channel (1080x1350).

Berisi:
- generate_portrait_bytes()  : Render poster ke BytesIO (in-memory, untuk Bot Broadcaster)

Menggunakan shared utilities dari poster_common.py.
"""
...
# ─── LAYOUT TELEGRAM CHANNEL (1080 x 1350) ──────────────────────────────────


def generate_portrait_bytes(job: JobDocument) -> Optional[io.BytesIO]:
    """Render poster Telegram Portrait (1080x1350) langsung ke dalam memori.

    Tidak menyimpan file ke disk. Gambar dirender di BytesIO dan siap
    dikirim langsung ke Telegram Bot API.

    Args:
        job: Dokumen loker dari MongoDB (berisi company, slug, location, dll).
    ...
    try:
        ...
        draw_icon_globe(draw, bx + icon_sz // 2, 70 + 15, icon_sz, ACCENT_BLUE)
        draw.text((bx + icon_sz + 10, 70), brand_txt, fill=ACCENT_BLUE, font=f_brand)

        # Logo Center
        if logo_img:
            logo_w, logo_h = logo_img.size
            img.paste(logo_img, ((W - logo_w) // 2, 230), logo_img)
        ...
        print(f"❌ Gagal render poster Telegram: {e}")
        return None
